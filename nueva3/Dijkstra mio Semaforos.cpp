...
class di{
	public:
		...
		void relajacion(Semaforo* , Semaforo*, int);
...
//	Lista l[];
   ...
   /* distancia[s->get_numero()]= 0;
    //cout<<a<<endl;
    	cout<<"Soy distancia de " << s->get_numero() << " y soy " <<* distancia<<endl;
    while(tamCola(c)!=0){
    	actual=desencolar(c);
    	cout<<"Soy actual "<<actual->get_numero()<<endl;
    	if(visitado[actual->get_numero()]) continue;
    	visitado[actual->get_numero()]=true;
    	for(int i=0;i<l[*actual->get_source()-1].getContador();i++){
    //		if( !visitado[actual->get_numero()] ){
			
    		adyacente=l[*actual->get_source()-1].destinos(i);
    		cout<<"Soy adyacente " << adyacente->get_numero()<<endl;
    		peso=l[*actual->get_source()-1].pesos(i);
    		cout<<"Soy peso "<< peso<<endl;
    		if(!visitado[adyacente->get_numero()]){
...
void di::relajacion(Semaforo* actual, Semaforo* adyacente, int peso){
	if(distancia[actual->get_numero()]+ peso < distancia[adyacente->get_numero()]){
		cout<<"Soy distancia antes de " << adyacente->get_numero() << " y soy " <<* distancia<<endl;
		distancia[adyacente->get_numero()] = distancia[actual->get_numero()] + peso;
		cout<<"Soy distancia de " << adyacente->get_numero() << " y soy " <<* distancia<<endl;
		previo[adyacente->get_numero()] = actual->get_numero();
		cout<<"Soy previo "<< *previo<<endl;
		encolar(c, adyacente);
			cout << "\n\n MOSTRANDO COLA\n\n";
                 if(c.delante!=NULL) muestraCola( c );
                 else   cout<<"\n\n\tCola vacia...!"<<endl;
